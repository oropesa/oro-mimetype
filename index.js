...
async function detectFile( magic, filepath ) {
  if( ! filepath || varType( filepath ) !== 'string' ) {
    return { status: false, filepath, msg: `Error: filepath is string required.` }
  }
...
async function isGenericType( genericType, magic, filepath ) {
  if( ! filepath || varType( filepath ) !== 'string' ) {
    return { status: false, filepath, failed: genericType, msg: `Error: filepath is string required.` }
  }

  if( ! genericType || varType( genericType ) !== 'string' ) {
    return { status: false, filepath, failed: genericType, msg: `Error: type is string required.` }
  }

  const response = await detectFile( magic, filepath );
  if( ! response.status ) { return response; }

  const regex = new RegExp( `^${genericType}/*` );
  const status = regex.test(response.mimetype);

  return status ? { status, filepath, mimetype: response.mimetype }
                : { status, filepath, mimetype: response.mimetype, failed: genericType, msg: `Error: File is not ${genericType}: '${filepath}'` }
}

async function isType( type, magic, filepath ) {
  if( ! filepath || varType( filepath ) !== 'string' ) {
    return { status: false, filepath, failed: [ type ], msg: `Error: filepath is string required.` }
  }
  ...
  return status ? { status, filepath, mimetype: response.mimetype }
                : { status, filepath, mimetype: response.mimetype, failed: type, msg: `Error: File is not type ${type}: '${filepath}'` }
}

async function isTypes( types, magic, filepath ) {
  if( ! filepath || varType( filepath ) !== 'string' ) {
    return { status: false, filepath, failed: types, msg: `Error: filepath is string required.` }
  }
  ...
  return status ? { status, filepath, mimetype: response.mimetype }
                : { status, filepath, mimetype: response.mimetype, failed: types, msg: `Error: File is not types ${types.join( ', ' )}: '${filepath}'` }
}
